# PST Testing Scenarios for ServiceTitan Connector

## Part A: Authentication & Connectivity Verification
- **Scenario A1: Valid Credential Connect**: `connect_servicetitan_connector` saves token and validates endpoint.
...
- **Scenario B1: List Projects**: `list_projects` returns typed list with bounds.
- **Scenario B2: Get ProjectRecord**: `get_projectrecord` returns entity details or 404 error.
- **Scenario B3: Health Audit**: `audit_projectrecord_health` aggregates active records.

## Part C: Safe Write & Idempotency Testing
- **Scenario C1: Connection Lifecycle**: Clean disconnect via `disconnect_servicetitan_connector` without lingering secrets.
