"""Connection lifecycle for ServiceTitan Connector."""
...
_SECRET = "servicetitan_connections"
...
@chat.function(
    "connect_servicetitan",
    "Connect ServiceTitan account via credentials.",
    action_type="write",
    chain_callable=True,
    event="servicetitan-connector.connect_servicetitan",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_servicetitan(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect ServiceTitan Connector."""
    client = ServicetitanClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "ServiceTitan Account",
...
@chat.function(
    "list_connections",
    "List connected ServiceTitan accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_servicetitan",
    "Disconnect ServiceTitan account.",
    action_type="write",
    chain_callable=True,
    event="servicetitan-connector.disconnect_servicetitan",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_servicetitan(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
